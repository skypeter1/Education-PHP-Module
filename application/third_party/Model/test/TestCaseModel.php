<?php
class TestCaseModel extends TestCase
{
	public function test_model_is_instanced_with_db_access()
	{
		$this->empty_tables();
		$database_access = DB();
		$model = new Model($database_access);

		$this->assert_true($model instanceof Model);
	}
	...
	public function test_model_extends_ci_model()
	{
		$database_access = DB();
		$model = new Model($database_access);
		$this->assert_true($model instanceof CI_Model);
	}
	...
	{
		$database_access = $this->get_database_access('educacion_development');
		$model = new TestingModel($database_access);
	...
	public function test_search_with_join()
	{
		$database_access = $this->get_database_access('educacion_development');

		$fixture_executor = new FixtureExecutioner(DB());
		...
		$join = new Join('users', 'users.id = entities.id', 'users.rol AS user_rol');
		
		$filter = new Filter('name', 'Entity 2');
		$search = new Search(array($filter), null, null, $join);

		$result = $model->search($search);
		$this->assert_true($result instanceof EntityCollection);
		$this->assert_equals(1, sizeof($result->collection));
		$this->assert_equals('profesor', $result->collection[2]['user_rol']);
	...
	public function test_search_with_concatenation()
	{
		$database_access = $this->get_database_access('educacion_development');
		$model = new TestingModel($database_access);
		...
		$concatenation = new Concatenation(array('entities.id','entities.name'), 'idname');
		$filter = new Filter('GLOBAL', '2Entity 2');
		$search = new Search(array($filter), null, null, null, $concatenation);
	...
	public function test_error_duplicated_property_on_create()
	{
		...
		catch(Error $exception)
		{
			...
			$this->assert_equals($model->error_duplicated_entity, $exception->getMessage());
		}
	...
	public function test_error_no_version_on_update()
	{
		...
		{
			$exception_thrown = true;
			$this->assert_equals($model->error_required_version, $exception->getMessage());
		}
	...
	public function test_concurrency_error_on_update()
	{
		...
		$first_user_entity = $model->get($entity_id);

		$second_user_entity = $model->get($entity_id);
		$second_user_entity->name = 'I am the second user';
		$model->update($second_user_entity);

		$first_user_entity->name = 'I am the first user';
		$exception_thrown = false;
		try
		{
			$model->update($first_user_entity);
		}
		catch(Error $exception)
		{
			$exception_thrown = true;
			$this->assert_equals($model->concurrency_error, $exception->getMessage());
		}
	...
	public function test_concurrency_error_on_delete()
	{
		...
		try
		{
			$model->delete($first_user_entity);
		}
		catch(Error $exception)
		{
			$exception_thrown = true;
